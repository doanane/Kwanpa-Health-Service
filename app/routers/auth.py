...
from app.auth.security import create_access_token, get_current_user  # Fixed import
...
async def signup(user_data: UserCreate, db: Session = Depends(get_db)):
    try:
        ...
        # Create new user - patient_id will be auto-generated
        hashed_password = get_password_hash(user_data.password)
        db_user = User(
            email=user_data.email,
            username=user_data.username,
            hashed_password=hashed_password
            # patient_id will be auto-generated by the event listener
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="User already exists or database constraint violated"
        )
...
async def login(login_data: UserLogin, db: Session = Depends(get_db)):
    try:
        ...
        # Ensure patient_id exists (for existing users)
